...
function getPolynom(...args) {
  return (x) => [...args].reverse().reduce((sum, n, i) => sum + n * x ** i, 0);
}
...
function memoize(func) {
  ...
  return function curried(...args) {
    if (args.length >= func.length) {
      const operation = JSON.stringify(args);
...
function logger(func, logFunc) {
  return (...args) => {
    const stringify = (f, arg) => `${f.name}(${JSON.stringify(arg).slice(1, -1)})`;
    const str = stringify(func, [...args]);
    logFunc(`${str} starts`);
 ...
 *   getId10() => 11
 */
function getIdGeneratorFunction(num) {
  let i = num - 1;
  return () => {
    i += 1;
    return i;
